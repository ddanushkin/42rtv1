...
void	trace_rays(t_app *app, int scene_id)
{
	...
	while (y < HEIGHT)
	{
		...
		while (x < WIDTH)
		{
			...
			t_vec shadow_orig = vec_dot(light_dir, nearest_hit.n) ? vec_sub(ray.origin, vec_mul_by(nearest_hit.n, 1e-3f)) : vec_add(nearest_hit.n, vec_mul_by(nearest_hit.n, 1e-3f));
